    ...
    if val == 0:
        ...
        if uploaded_file_train is not None:
            for i in uploaded_file_train:
                data = copy.deepcopy(i)
                df_train = pd.read_csv(i)
                st.subheader("Data preview - {}".format(i.name))
                st.write(df_train.head())
                
                
        response = requests.post("http://localhost:8000/uploadfiles", files=uploaded_file_train)
        ...
        select = st.selectbox("Choose a file to preprocess", file_list)
